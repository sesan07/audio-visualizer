...
export class CircleContent extends BaseContent<ICircleContentConfig> {
    ...
    protected _animateContent(entity: IEntityConfig<ICircleContentConfig>): void {
        ...
        for (let i = 0; i < config.amplitudes.length; i++) {
            ...
            this._drawCircle(xLeft, y, gradientColor, config.startColor, config.sampleRadius * this._scale, entity.opacity);
            this._drawCircle(xRight, y, gradientColor, config.startColor, config.sampleRadius * this._scale, entity.opacity);
        }
    }

    private _drawCircle(centerX: number, centerY: number, fillColor: RGB, strokeColor: RGB, sampleRadius: number, opacity: number): void {
